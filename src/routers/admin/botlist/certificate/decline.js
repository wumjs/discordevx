...
app.post("/admin/certificate/delete/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<:emoji_74:936700721166962708> <@${botdata.ownerID}>'s bot named **${bota.tag}** has not been granted a certificate.`)
        client.users.cache.get(botdata.ownerID).send(`<:emoji_74:936700721166962708> Your bot named **${bota.tag}** certificate application has been declined.\nReason: **${rBody['reason']}**`)
    });
