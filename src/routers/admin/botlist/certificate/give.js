...
app.get("/admin/certificate/give/:botID", global.checkAuth, async (req, res) => {
  ...
  client.users.fetch(botdata.botID).then(bota => {
    client.channels.cache.get(global.config.server.channels.botlog).send(`<:emojiArr:936700599121088522> <@${botdata.ownerID}>'s bot named **${bota.tag}** has been granted a certificate.`)
    client.users.cache.get(botdata.ownerID).send(`<:emojiArr:936700599121088522> Your bot named **${bota.tag}** has been certified.`)
  });
