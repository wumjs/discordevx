...
app.get("/admin/confirm/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(req.params.botID).then(bota => {
        client.channels.cache.get(global.config.server.channels.botlog).send(`<:apppr:945714654720692254> <@${botdata.ownerID}>'s bot named **${bota.tag}** has been approved by <@${req.user.id}>.`)
        client.users.cache.get(botdata.ownerID).send(`<:apppr:945714654720692254> Your bot named **${bota.tag}** has been approved by <@${req.user.id}>.`)
    });
